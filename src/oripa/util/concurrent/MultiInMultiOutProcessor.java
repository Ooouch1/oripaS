...
public class MultiInMultiOutProcessor<Value, Output> {
	...
	{	
		...
		if(false == (values instanceof RandomAccess)){
			ArrayList<List<Value>> separated = separate(values, divNum);
			return execute(separated);
		}

		// which is faster???
		
		ExecutorService executor;
		executor = Executors.newFixedThreadPool(divNum);
		...
		try{
			int div = ((values.size() % divNum) == 0)? divNum: divNum-1; 
			final int subListSize = values.size() / div;
			int from = 0;
			int to = subListSize;
			
			for(int i = 0; i < divNum; i++){
				completion.submit( processFactory.create().getCallable(values.subList(from, to)) );
				from = to;
				to += subListSize;
				if(to > values.size()){
					to = values.size();
	...
	public ArrayList<Output> execute(final List< List<Value> > valueSets) {
		int divNum = -1;
		ExecutorService executor;
...
//		}
	 ...
	 */
	public ArrayList<List<Value>> separateKeepingOrder(final List<Value> values, int divNum){

		ArrayList<List<Value>> valueSets = new ArrayList<>();
	 ...
	 * @return a list of value collections as the result of separation.
	 */
	public ArrayList<List<Value>> separate(final List<Value> values, int divNum){
	 ...
	 * @return a list of value collections as the result of separation.
	 */
	private Iterator<Value> separateEqually(final List<Value> values, ArrayList<List<Value>> valueSets, int divNum){
